...
function FormCreate() {
  ...
  const handleChange = (e) => {
    ...
    const updatedValue = name === 'price' || name === 'sale' || name === 'quantity' ? Number(value) : value;
  ...
  const validate = (values) => {
    ...
    if (!values.price) {
      ...
      // errors.price = "This is not a valid number format!";
    }
    if (!values.image) {
  ...
  useEffect(() => {
    (async function () {
      ...
      if (Object.keys(formErrors).length === 0 && isSubmit) {
        const currentTimestamp = moment().format('YYYYMMDDHHmmss'); // Get the current timestamp as a string
        const productsRef = ref(db, 'products');
        const newProductRef = child(productsRef, currentTimestamp);
        try {
          const newProductId = currentTimestamp;
